...
const STYLES = {
	...
	desktop: {
		...
		title:
			"font-bold text-2xl lg:text-4xl-custom leading-8 lg:leading-10 tracking-tight truncate",
		statusBadge:
			"flex items-center space-x-2 bg-forest-green-9/10 rounded-2xl px-2 py-1 flex-shrink-0",
	...
	mobile: {
		...
		statusBadge:
			"flex items-center space-x-2 bg-forest-green-9/10 rounded-2xl px-2 py-1",
	},
	profiles: {
		container: "flex items-center gap-x-2 lg:gap-x-3 mt-4 lg:my-6",
		avatarGroup: "flex -space-x-2 lg:-space-x-3",
		namesContainer:
			"flex gap-x-1 items-center text-brick-brown-62/62 leading-5 tracking-[-0.005em] text-sm lg:text-base min-w-0",
	},
	buttons: {
		share:
		...
		<header className={STYLES.container}>
			...
			<div className={STYLES.desktop.container}>
				<div className={STYLES.desktop.titleSection}>
					<div className={STYLES.desktop.titleWrapper}>
						...
						<button
							...
							className="flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 rounded-sm"
						>
							<Image
								src="/caret-down.svg"
								alt=""
					...
					<div
						className={STYLES.desktop.statusBadge}
						role="status"
						aria-label="Wallet status: Active"
					>
						<Image
							src="/status-dot.svg"
							alt=""
				...
				<div className={STYLES.desktop.actions}>
					<button
						...
						className={STYLES.buttons.share}
					>
						Share
					</button>
					<button
			...
			<div className={STYLES.mobile.container}>
				<div className={STYLES.mobile.titleRow}>
					<div className={STYLES.mobile.titleWrapper}>
						...
						<button
							type="button"
							aria-label="Wallet options"
							className="flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-foreground focus:ring-offset-2 rounded-sm"
						>
							<Image
								src="/caret-down.svg"
								alt=""
				...
				<div className={STYLES.mobile.statusRow}>
					<div
						className={STYLES.mobile.statusBadge}
						role="status"
						aria-label="Wallet status: Active"
					>
						<Image
			...
			<div className={STYLES.profiles.container}>
				<div
					className={STYLES.profiles.avatarGroup}
					role="group"
				...
				>
					{TEAM_PROFILES.map((profile, index) => (
						<div
							key={profile.id}
							className={getProfileVisibility(index)}
							style={getProfileZIndex(index)}
						>
							<Image
								src={profile.image}
								...
								width={32}
								height={32}
								className="w-8 h-8 lg:size-[41px] rounded-full"
							/>
						</div>
					))}
				</div>
